...
def remember_login(username):
    """Keep auth in the browser session only; do not use a shared server-side remember file.
    A global remember file leaks one user's account across everyone using the same deployed app.
    """
    return None


def load_remembered_login():
    """Disabled for multi-user deployments. Each browser/session should stay isolated."""
    return None
