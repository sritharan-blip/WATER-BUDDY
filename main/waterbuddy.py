    ...
    with st.container():
        ...
        with cols[1]:
            ...
            if cols[1].button('🎨 Customize', key='home_customize', use_container_width=True):
                ...
                st.rerun()
    st.markdown("<div style='height:20px;'></div>", unsafe_allow_html=True)
    st.markdown("<div class='section-title'>💡 Hydration Tips</div>", unsafe_allow_html=True)
    tip_cols = st.columns(2)
    for index, tip in enumerate(HYDRATION_TIPS[:2]):
        with tip_cols[index]:
            st.markdown(f"<div class='glass-panel-alt' style='min-height:110px;'><div style='font-weight:700; color:{theme['accent']};'>Tip {index + 1}</div><div style='color:{theme['muted']}; margin-top:10px;'>{tip}</div></div>", unsafe_allow_html=True)
...
def render_shop(theme: dict[str, str]) -> None:
    ...
    for item in REWARD_ITEMS:
        ...
        with item_columns[REWARD_ITEMS.index(item) % 3]:
            st.markdown(
                f"<div class='achievement-card'><div style='display:flex; justify-content:space-between; gap:12px;'><strong>{item['name']}</strong><strong style='color:{theme['accent']};'>{item['cost']} 💎</strong></div><div style='color:{theme['muted']}; margin-top:6px;'>{item['category']} • {item['rarity']}</div></div>",
                unsafe_allow_html=True,
            )
            if not owned:
