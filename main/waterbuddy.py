...
# Temporary QA balance; remove this block after the shop test is complete.
TEMP_TEST_COIN_GRANT = 10_000
TEMP_TEST_COIN_FLAG = "temporary_test_wallet_granted_v1"
...
REWARD_ITEMS = [
    ...
    {"id": "mythic_webwalker", "name": "Mythic Webwalker", "cost": 5000, "category": "Skin", "rarity": "MYTHIC"},
...
def get_buddy_avatar_html(buddy: dict[str, Any], theme: dict[str, str], progress_percent: int = 0) -> str:
    ...
    elif skin == "mythic_webwalker":
        gradient_stops = '<stop offset="0%" stop-color="#42e8ff"/><stop offset="46%" stop-color="#1168d9"/><stop offset="72%" stop-color="#d92755"/><stop offset="100%" stop-color="#160b3a"/>'
    else:
        gradient_stops = '<stop offset="0%" stop-color="#baf4ff"/><stop offset="55%" stop-color="#39c9f0"/><stop offset="100%" stop-color="#087ebd"/>'
    cosmic_details = ""
    if skin == "mythic_webwalker":
        cosmic_details = """
                <circle cx="44" cy="48" r="12" fill="#ffe88a" stroke="#ff6a9d" stroke-width="4"/>
                <circle cx="218" cy="55" r="8" fill="#8ff5ff" stroke="#345dff" stroke-width="4"/>
                <circle cx="225" cy="210" r="16" fill="#ffce70" stroke="#ee4e65" stroke-width="5"/>
                <ellipse cx="225" cy="210" rx="27" ry="8" fill="none" stroke="#ffe9a8" stroke-width="3" transform="rotate(-18 225 210)"/>
                <path d="M16 122 Q130 28 244 122 M18 158 Q130 72 242 158 M48 36 Q130 130 212 36 M30 210 Q130 130 230 210" fill="none" stroke="#d7f8ff" stroke-width="2" opacity=".62"/>
                <path d="M130 24 L130 232 M62 58 Q130 130 198 58 M38 112 Q130 130 222 112 M38 172 Q130 130 222 172" fill="none" stroke="#8eeeff" stroke-width="2" opacity=".55"/>
                <path d="M90 94 L130 132 L170 94 L154 170 L130 192 L106 170 Z" fill="#d91f4d" fill-opacity=".94" stroke="#ffb0c4" stroke-width="3"/>
                <path d="M130 132 L130 192 M90 94 L170 94 M106 170 L154 170" fill="none" stroke="#5be8ff" stroke-width="3"/>
                <text x="26" y="84" font-size="20" fill="#ffffff">✦</text>
                <text x="202" y="92" font-size="17" fill="#ffffff">✧</text>
                <text x="28" y="232" font-size="23" fill="#ffe68a">✦</text>
        """
    face = {
...
def init_state() -> None:
    if "data" not in st.session_state:
        ...
        if not data["stats"].get(TEMP_TEST_COIN_FLAG):
            data["stats"]["coins"] = max(data["stats"].get("coins", 0), TEMP_TEST_COIN_GRANT)
            data["stats"][TEMP_TEST_COIN_FLAG] = True
    ...
    with st.container():
        ...
        with cols[1]:
            ...
            if cols[0].button('💬 Talk', key='home_talk', use_container_width=True):
                query_page('Buddy')
                st.rerun()
            if cols[1].button('🎨 Customize & Shop', key='home_customize', use_container_width=True):
                query_page('Buddy')
                st.rerun()
    st.markdown("<div style='height:20px;'></div>", unsafe_allow_html=True)
...
def render_shop(theme: dict[str, str]) -> None:
    st.markdown("<div id='buddy-shop'></div>", unsafe_allow_html=True)
...
def render_buddy(theme: dict[str, str]) -> None:
    ...
    with action_cols[1]:
        st.markdown(
            "<a class='glass-button' href='#buddy-shop' style='width:100%; box-sizing:border-box;'>🎨 Customize & Shop</a>",
            unsafe_allow_html=True,
        )
    status_html = f"""
